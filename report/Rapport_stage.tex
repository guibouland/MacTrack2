...
\chapter{Segmentation automatique \og framewise\fg\ pour le suivi de la réponse calcique dans les macrophages chez le zebrafish}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=.4\linewidth]{imgs/001_frame.png}
    \includegraphics[width=.4\linewidth]{imgs/001_frame_roi.png}
    \caption{}
    \label{mac:train}
\end{figure}
...
\begin{table}[!ht]
    ...
    \end{tabular}
    \caption{Exemples de performances des modèles entraînées sur les différents jeux de données.\\[-0.3em] {\footnotesize Les valeurs sont les IoU moyens sur le jeu de données d'entraînement et de test. Le modèle dont les performances sont écrites en rouge est celui que nous avons conservé pour la suite, et que nous avons fourni en exemple dans le dépôt GitHub du projet.}}
    \label{mac:dataset}
\end{table}
...
\begin{figure}[!ht]
    ...
    \caption{Exemples de comparaisons entre la vérité annotée (en bleu) et les prédictions du modèle choisi (en rouge) sur des images issues de l'échantillon d'entraînement (à gauche) et de test (à droite).\\[-0.3em]
    {\scriptsize\begingroup\linespread{0.2} Le modèle choisi est celui écrit en rouge dans la \tabref{mac:dataset}. Pour l'image d'entraînement, l'IoU est de 0.67 et pour l'image de test, l'IoU est de 0.76. Ce sont toutes les deux des images issues du jeu de données \og petit+crop\fg.\endgroup}}
    \label{mac:viz_model}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption{Illustrations des différentes sorties de la fonction \texttt{locate}, au milieu pour la segmentation complète et à droite pour la segmentation objet par objet, par rapport à l'image d'origine (à gauche).\\[-0.3em]
    {\scriptsize\setstretch{0.2} Ces images proviennent de l'observation de la nageoire caudale d'un poisson à 3h post-amputation de celle-ci. Il s'agit de la deuxième image de la vidéo et des segmentations complète et d'un objet de cette même image.}
    }
    \label{mac:locate}
\end{figure}
...
\subsection{Limitations <- à enlever / modifier}
