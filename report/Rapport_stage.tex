...
Des études récentes \cite{mehari} ont montré que les signaux de calcium intracellulaires auraient un rôle dans l'activation des macrophages M1. Cependant, le lien entre les signaux calciques et la polarisation reste flou. En effet, nous ne connaissons pas les schémas de flux calcique entre les différents états fonctionnels des macrophages. L'étude de ces signaux calciques semble être une voie prometteuse et pourrait permettre d'identifier ces états.

\
% Données : microcopie confocale. Suivie longitudinale apres une inflamation.

description générale des macro et polarisation, on ne connait pas les schéma de flux entre les états

zebrafish, créer des métriques pertinentes pour l'étue des signaux calciques

ce qu'on produit: librairie de segmentation et tracking automatique des macro pour en extraire les données calciques

exploration d'une piste plus générale à travers modèle de fondation SAM-2

production d'une analyse statistique sur les données de calcium

ce à quoi ça va servir

lien vers les objectifs
...
\
% 2eme etape : un film est un enchaineement d'images. Chaque image étant segmentée, il faut suivre les objets: fine tuning de Sam2
...
\section{Macrophages, cellules immunitaires}
polarisation, M1, M2, pro-inflammatoire, anti-inflammatoire \dots

\section{Signal calcique}

\section{Zebrafish, modèle animal idoine}

\section{Microscopie confocale et lignée transgénique}
