...
\begin{figure}[!ht]
    ...
    {\setstretch{0.2}\caption*{\scriptsize Les macrophages sont observés, de gauche à droite, à 3, 6, 24, 48 et 72 heures post-amputation (hpa). Il s'agit d'un poisson dont la nageoire caudale a été coupée (ne faisait pas partie du groupe contrôle).}}
    \label{mac:macros}
\end{figure}
...
\section{Segmentation automatique d'images avec MacTrack2}
...
En effet, Kartezio s'attend à ce que les images d'entraînement soient présentes dans un dossier \og train\fg\ qui contient les images (dossier \texttt{train\_x}) et les fichiers ZIP (dossier \texttt{train\_y}) de chaque image contenant les annotations des macrophages sous la forme de fichiers ROI. En revanche pour notre vidéo, les images extraites de celle-ci, pour le canal rouge qui nous est utile pour la segmentation des macrophages, sont placées dans le dossier \og test\fg, qui contient les images de la vidéo (dossier \texttt{test\_x}), mais dans le dossier \texttt{test\_y}, il n'y a pas de fichiers ZIP, car les vidéos que l'on cherche à segmenter automatiquement n'ont pas été annotées à la main. Cependant, pour la structure de Kartezio, il s'attend à ce qu'ils y ait des fichiers ZIP, au même nombre que les images dans le dossier correspondant. C'est pour cela que nous ajoutons automatiquement des fichiers ZIP vides dans le dossier \texttt{test\_y}. Nous avons aussi besoin d'un fichier \texttt{dataset.csv}~(\tabref{mac:dataset_csv}) indiquant les images, qu'elles viennent de l'entraînement ou du test qui n'en est pas vraiment un, ainsi que les masques associés même s'ils sont vides, et finalement une colonne \og set\fg\ qui indique si l'image est une image d'entraînement ou de test (donc issue de la vidéo).\\
