...
\chapter{Segmentation automatique \og \textit{frame-wise}\fg\ de vidéos pour le suivi temporel de la réponse calcique dans les macrophages chez le zebrafish}
...
$$\text{IoU}_k(x_k,y_k) = \frac{|x_k\cap y_k|}{|x_k\cup y_k|}$$

\noindent avec $x_k$ la prédiction de l'objet $k$ et $y_k$ la vérité annotée, et $|\cdot |$ la mesure de l'aire. Pour obtenir la valeur de l'IoU pour une image, on fait la moyenne sur tous les objets segmentés dans l'image :

\begin{align}\label{eq:iou}
    \text{mIoU}(x,y) = \frac{1}{K}\sum_{k=1}^{K}\text{IoU}_k(x_k,y_k)
\end{align}

\noindent où $K$ est le nombre total d'objet dans une image, $x$ l'ensemble des prédictions dans l'image et $y$ l'ensemble des vérités annotées.
...
Les masques sont données sous la forme de fichiers ROI (Region Of Interest), qui sont présentés dans la \figref{mac:train}, compilés sous un fichier ZIP. Ainsi il y a un fichier ZIP par image et un fichier ROI par macrophage détouré.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
        \hline
        \multicolumn{2}{|c}{Modèle initial} & \multicolumn{2}{|c}{Modèle petit} & \multicolumn{2}{|c}{Modèle crop} & \multicolumn{2}{|c|}{Modèle petit+crop}                                                                   \\ \hline
        \textbf{Train}                      & \textbf{Test}                     & \textbf{Train}                   & \textbf{Test}                           & \textbf{Train} & \textbf{Test} & \textbf{Train} & \textbf{Test} \\ \hline
        0.659                               & 0.663                             & 0.639                            & 0.628                                   & 0.674          & 0.706         & \red{0.712}    & \red{0.763}   \\\hline
    ...
    \vspace{-1em}{\setstretch{0.2}\caption*{\scriptsize Les valeurs sont les IoU moyens sur le jeu de données d'entraînement et de test. Le modèle dont les performances sont écrites en rouge est celui que nous avons conservé pour la suite, et que nous avons fourni en exemple dans le dépôt GitHub du projet.}}
    \label{mac:dataset}
\end{table}
...
\begin{figure}[!ht]
    ...
    \caption{Illustrations des différentes sorties de la fonction \texttt{locate}, au milieu pour la segmentation complète et à droite pour la segmentation objet par objet, par rapport à l'image d'origine (à gauche).}
    \vspace{-1em}
    {
        \setstretch{0.2}
        \caption*{\scriptsize Ces images proviennent de l'observation de la nageoire caudale d'un poisson à 3h post-amputation de celle-ci. Il s'agit de la deuxième image de la vidéo et des segmentations complète et d'un objet de cette même image.}
    }
    \label{mac:locate}
\end{figure}
...
Tracking (calcul iou paire par paire de frame pour tous les objets, avec un seuil 0.5 car pas la meme frame et les macro bougent beaucoup donc seuil bas pour un bon compromis, si >0.5, alors on considère la paire comme etant le meme et on les numérote pareil)
...
En moyennant sur tous les objets $k$ d'une image, on obtient la mIoU (IoU moyenne), comme obtenue par~\ref{eq:iou} dans la partie~\ref{evalperf}
